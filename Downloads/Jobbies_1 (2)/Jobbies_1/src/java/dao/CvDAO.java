...
public class CvDAO extends GenericDAO<CV> {
   ...
   public int insert(CV cv) {
    // Đảm bảo FilePath được gán chuỗi rỗng '' thay vì NULL nếu DB yêu cầu NOT NULL
    String sql = "INSERT INTO CVs(JobSeekerID, Title, TemplateCode, IsDefault, Summary, Skills, Links, FilePath, UploadDate, LastUpdated) " +
    ...
    try (PreparedStatement ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
        ...
        if (affectedRows > 0) {
            try (ResultSet keys = ps.getGeneratedKeys()) {
                if (keys.next()) {
                    return keys.getInt(1); // Trả về ID thật từ DB
                }
            }
        }
    } catch (Exception e) {
        e.printStackTrace(); // Lỗi SQL sẽ hiện ở tab Output/Console của NetBeans
    }
    return -1; // Trả về -1 nếu thất bại hoàn toàn
}
...
//nhap cv
    ...
    public int insertUploaded(int jobSeekerId, String title, String filePath) throws SQLException {
        ...
        "VALUES(?, ?, ?, 'UPLOADED', 0, NULL, NULL, NULL, SYSDATETIME(), SYSDATETIME())";
