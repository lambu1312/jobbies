...
/**
 * Download / Preview CV PDF
 * - /cv/download?cvid=38              => download
 * - /cv/download?cvid=38&preview=1    => preview (inline)
 *
 * Fixes included:
 * - Build PDF bytes in memory BEFORE writing response to avoid:
 *   IllegalStateException: Cannot call reset() after response has been committed
 * - Always call CvSectionDAO.listByCv(cvid, jobSeekerId) (match required: int,int)
 * - Underline lines are placed BELOW titles (no longer crossing text)
...
public class DownloadCVServlet extends HttpServlet {
    ...
    if (preview) resp.setHeader("Content-Disposition", "inline; filename=\"CV_" + cvid + ".pdf\"");
    else resp.setHeader("Content-Disposition", "attachment; filename=\"CV_" + cvid + ".pdf\"");
    ...
    private PDFont loadFont(PDDocument doc, String webPath) throws IOException {
        try (InputStream in = getServletContext().getResourceAsStream(webPath)) {
            if (in == null) throw new IOException("Font not found in webapp: " + webPath);
    ...
    /**
     * Avatar supports:
     * - stored path: /JobSeeker/images/xxx.jpg or /images/xxx.jpg
     * - URL: https://...
     */
    private PDImageXObject loadAvatarImage(PDDocument doc, HttpServletRequest req, Account acc) {
        try {
            ...
            if (!ctx.isEmpty() && a.startsWith(ctx + "/")) a = a.substring(ctx.length());
            if (a.startsWith("/JobSeeker/")) a = a.substring("/JobSeeker".length());
            if (!a.startsWith("/") && !a.startsWith("http")) a = "/" + a;
    ...
    private static class PageCtx {
        ...
        PageCtx(PDDocument doc) { this.doc = doc; }
    ...
    private void renderTemplate2(PDDocument doc,
        ...
        while (blockIndex < blocks.size()) {
            ...
            if (isFirstPage) {
                ...
                if (avatar != null) p.cs.drawImage(avatar, avatarX, avatarY, avatarSize, avatarSize);
                else drawRect(p.cs, avatarX, avatarY, avatarSize, avatarSize, false);
            ...
            while (blockIndex < blocks.size()) {
                ...
                p.y -= 10;
                blockIndex++;
                if (p.y < BOTTOM_MARGIN + 90) break; // Hết giấy, sang trang mới
            }
    ...
    private static class SectionBlock {
        String title;
        String body;
        SectionBlock(String t, String b) { title = t; body = b; }
    }
    ...
    private float wrapTextPaged_SIDEBAR(PageCtx p, PDFont font, int size,
        ...
        for (String para : paragraphs) {
            if (y < minY) return y;
            if (para.isBlank()) { y -= (size + 3); continue; }
    ...
    /**
     * Wrap text cho Sidebar có khả năng tràn trang (Flow).
     * Trả về Object[]{ float currentY, String remainingText }
     */
    private Object[] wrapTextSidebarFlow(PageCtx p, PDFont font, int size,
                                         float x, float y, float width, String text) throws IOException {
        if (text == null || text.isBlank()) return new Object[]{y, ""};

        float minY = BOTTOM_MARGIN + 10;
        StringBuilder remaining = new StringBuilder();
        boolean overflow = false;

        // Nếu ngay từ đầu đã hết chỗ
        if (y < minY) return new Object[]{y, text};
        ...
        for (int i = 0; i < paragraphs.length; i++) {
            ...
            // Nếu đã bị tràn ở các dòng trước, gom hết phần còn lại vào remaining
            if (overflow) {
                remaining.append(para).append("\n");
                continue;
            }

            // Xử lý đoạn văn hiện tại
            if (para.isBlank()) {
                y -= (size + 3);
                if (y < minY) {
                    overflow = true; // Phát hiện tràn ngay tại dòng trống
                }
                continue;
            }
            ...
            for (int k = 0; k < words.length; k++) {
                String w = words[k];
                // Nếu đã tràn, thêm từ vào remaining
                if (overflow) {
                    // Nếu line còn dở dang thì thêm vào trước
                    if (line.length() > 0) {
                         remaining.append(line).append(" "); 
                         line.setLength(0);
                    }
                    remaining.append(w).append(" ");
                    continue;
                }
                ...
                if (testWidth > width) {
                    ...
                    if (y < minY) {
                        overflow = true;
                        remaining.append(line).append(w).append(" "); // Lưu lại từ chưa in
                        continue;
                    }
                    writeText(p.cs, font, size, x, y, line.toString().trim());
                    y -= (size + 3);
            ...
            // In nốt phần còn lại của đoạn văn (nếu chưa tràn)
            if (!overflow && line.length() > 0) {
                ...
                // Đã tràn, lưu nốt vào buffer
                remaining.append(line).append("\n");
            }

            // Xuống dòng giữa các đoạn văn (nếu chưa tràn và chưa phải đoạn cuối)
            if (!overflow) {
                remaining.append("\n"); // Thêm xuống dòng ảo để tách đoạn nếu flow tiếp
            }
        }
