...
public class ApplicationDAO extends GenericDAO<Applications> {
    ...
    public List<Applications> findApplicationByCandidateID(int id, int page, int pageSize) {
        String sql = "select * from Applications where CandidateID = ? ORDER BY ApplicationID OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
        parameterMap = new LinkedHashMap<>();
    ...
    public List<Applications> findApplicationByCandidateID(int id) {
        String sql = "select * from Applications where CandidateID = ?";
        parameterMap = new LinkedHashMap<>();
    ...
    public List<Applications> findApplicationByJobPostingID(int id) {
        ...
        CandidateDAO jobSeekerDao = new CandidateDAO();
        for (Applications application : applications) {
            application.setCandidate(jobSeekerDao.findCandidateIDByCandidateID(application.getCandidateID() + ""));
        }
        return applications;
    }
    ...
    public List<Applications> findApplicationByCandidateIDAndStatus(int jobSeekerID, int status, int page, int pageSize) {
        String sql = "SELECT * FROM Applications WHERE CandidateID = ? AND Status = ? ORDER BY ApplicationID OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
    ...
    public Applications findPendingApplication(int jobSeekerID, int jobPostingID) {
        String sql = "SELECT * FROM Applications WHERE CandidateID = ? AND JobPostingID = ? AND Status = 3";
        parameterMap = new LinkedHashMap<>();
        parameterMap.put("CandidateID", jobSeekerID);
    ...
    public int countTotalApplicationsByJSIDAndStatus(int jobSeekerID, int status) {
        String sql = "SELECT count(*) FROM [dbo].[Applications] WHERE CandidateID = ? AND Status = ?";
        parameterMap = new LinkedHashMap<>();
        parameterMap.put("CandidateID", jobSeekerID);
        parameterMap.put("Status", status);
    ...
    public int countTotalApplicationsByJSID(int jobSeekerID) {
        String sql = "SELECT count(*) FROM [dbo].[Applications] WHERE CandidateID = ?";
        parameterMap = new LinkedHashMap<>();
        parameterMap.put("CandidateID", jobSeekerID);
        return findTotalRecordGenericDAO(Applications.class, sql, parameterMap);
    }
    ...
    public List<Applications> findApplicationsByFilters(int jobPostingId, String searchName, String statusFilter, String dateFilter, int page, int pageSize) {
        StringBuilder sql = new StringBuilder("SELECT A.* FROM Applications as A"
                + " Join Candidates as J on A.CandidateId = J.CandidateId"
