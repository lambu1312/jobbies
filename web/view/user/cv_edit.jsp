...
<head>
    ...
    <title><c:choose><c:when test="${cv == null}">Tạo CV Mới</c:when><c:otherwise>Chỉnh Sửa CV</c:otherwise></c:choose></title>
        ...
        <div class="form-card">
            ...
            <div class="form-group">
                ...
                <textarea name="summary" id="summary" rows="5" class="form-control" 
                          placeholder="Giới thiệu ngắn gọn về bản thân, kinh nghiệm và mục tiêu nghề nghiệp..."><c:out value="${cv != null ? cv.summary : ''}"/></textarea>
            ...
            <div class="form-group">
                ...
                <textarea name="skills" id="skills" rows="3" class="form-control" 
                          placeholder="VD: Microsoft Office, Marketing, SEO, Photoshop, ..."><c:out value="${cv != null ? cv.skills : ''}"/></textarea>
            ...
            <div class="form-group">
                ...
                <textarea name="links" id="links" rows="4" class="form-control" 
                          placeholder="Mỗi dòng một link hoặc chứng chỉ..."><c:out value="${cv != null ? cv.links : ''}"/></textarea>
                            ...
                            <div class="section-card-body">
                                ...
                                <div class="form-group" style="margin-bottom: 0;">
                                    ...
                                    <textarea name="sectionContent" rows="6" class="form-control" 
                                              placeholder="Nhập nội dung chi tiết..."><c:out value="${s.content}"/></textarea>
                                </div>
                            </div>
            ...
            <div class="form-actions">
                ...
                <c:if test="${cv != null}">
                    <div class="link-group">
                        <a class="link-action" href="${pageContext.request.contextPath}/cv/preview?cvid=${cv.cvId}" target="_blank">
                            <i class="fas fa-eye"></i>
                            Xem Trước
                        </a>
                        <a class="link-action" href="${pageContext.request.contextPath}/cv/download?cvid=${cv.cvId}">
...
function validateForm(event) {
    ...
    // Regex pattern: không cho phép chuỗi chỉ có khoảng trắng hoặc rỗng
    const notEmptyRegex = /\S+/;
    
    // Validate Title
    if (!notEmptyRegex.test(title.value)) {
        showError(title, 'title-error');
        isValid = false;
    } else {
        hideError(title, 'title-error');
    }
    
    // Validate Summary
    if (!notEmptyRegex.test(summary.value)) {
        showError(summary, 'summary-error');
        isValid = false;
    } else {
        hideError(summary, 'summary-error');
    }
    
    // Validate Skills
    if (!notEmptyRegex.test(skills.value)) {
        showError(skills, 'skills-error');
        isValid = false;
    } else {
        hideError(skills, 'skills-error');
    }
    
    // Validate Links
    if (!notEmptyRegex.test(links.value)) {
        showError(links, 'links-error');
        isValid = false;
    } else {
        hideError(links, 'links-error');
    }
    ...
    fields.forEach(item => {
        ...
        if (field) {
            field.addEventListener('input', function() {
                const notEmptyRegex = /\S+/;
                if (notEmptyRegex.test(this.value)) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Setup input listeners to remove errors on typing
    setupInputListeners();
});
