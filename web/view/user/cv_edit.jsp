...
<html lang="vi">
    <head>
        ...
        <script>
            // ===== Validation helpers =====
            const notEmptyRegex = /\S+/; // có ít nhất 1 ký tự không phải khoảng trắng
            const MAX_WORDS = 124;
            ...
            function validateForm(event) {
                ...
                const fields = [
                    ...
                    {id: 'summary', error: 'summary-error'},
                    {id: 'skills', error: 'skills-error'},
                    {id: 'links', error: 'links-error'}
                ];
                ...
                fields.forEach(item => {
                    ...
                    if (!notEmptyRegex.test(value)) {
                        showError(field, item.error, "Không được để trống");
                        isValid = false;
                    } else if (words > MAX_WORDS) {
                        showError(field, item.error, `Không được vượt quá 124 từ`);
                        isValid = false;
                    } else {
                        hideError(field, item.error);
                    }
                });
            ...
            function setupInputListeners() {
                ...
                fields.forEach(item => {
                    ...
                    field.addEventListener('input', function () {
                        ...
                        if (!notEmptyRegex.test(value)) {
                            showError(this, item.error, "Không được để trống");
                        } else if (words > MAX_WORDS) {
                            showError(this, item.error, `Không được vượt quá 124 từ`);
                        } else {
